...
::testing::AssertionResult TimeEqual(const fort::Time &a, const fort::Time &b) {
	if (a.Equals(b) == false) {
		return ::testing::AssertionFailure()
		       << "a: " << a.DebugString() << "b: " << b.DebugString()
	...
	if (google::protobuf::util::MessageDifferencer::Equals(
		...
		return ::testing::AssertionFailure()
		       << "a: " << a.DebugString() << "b: " << b.DebugString()
		       << " and a.Timestamp() and b.Timestamp() yield different "
		          "results";
	}
...
TEST_F(TimeUTest, DurationParsing) {
	...
	std::vector<TestData> data = {
	    ...
	    {"100.00100s",
	     true,
	     100 * Duration::Second + 1 * Duration::Millisecond},
	    // different units
	    ...
	    {"10.5s4m",
	     true,
	     4 * Duration::Minute + 10 * Duration::Second +
	         500 * Duration::Millisecond},
	    {"-2m3.4s",
	     true,
	     -(2 * Duration::Minute + 3 * Duration::Second +
	       400 * Duration::Millisecond)},
	    {"1h2m3s4ms5us6ns",
	     true,
	     1 * Duration::Hour + 2 * Duration::Minute + 3 * Duration::Second +
	         4 * Duration::Millisecond + 5 * Duration::Microsecond +
	         6 * Duration::Nanosecond},
	    {"39h9m14.425s",
	     true,
	     39 * Duration::Hour + 9 * Duration::Minute + 14 * Duration::Second +
	         425 * Duration::Millisecond},
	    // large value
	    ...
	    {"9007199254740993ns",
	     true,
	     ((int64_t(1) << 53) + int64_t(1)) * Duration::Nanosecond},
	    ...
	    {"-9223372036854775807ns",
	     true,
	     Duration(-1L << 63) + 1 * Duration::Nanosecond},
	    // huge string; issue 15011.
	    {"0.100000000000000000000h", true, 6 * Duration::Minute},
	    // This value tests the first overflow check in leadingFraction.
	    {"0.830103483285477580700h",
	     true,
	     49 * Duration::Minute + 48 * Duration::Second +
	         372539827 * Duration::Nanosecond},
	...
	for (const auto &d : data) {
		...
		if (d.OK == true) {
			try {
				...
				ADD_FAILURE() << "Unexpected exception while parsing '"
				              << d.Input << "': " << e.what();
			}
		} else {
			try {
				auto res = Duration::Parse(d.Input);
				ADD_FAILURE()
				    << "Should have received an exception while parsing '"
				    << d.Input << "' but got result: " << res.Nanoseconds();
			} catch (const std::exception &e) {
			}
		}
	}
...
TEST_F(TimeUTest, DurationFormatting) {
	...
	std::vector<TestData> data = {
	    ...
	    {"5h6m7.001s",
	     5 * Duration::Hour + 6 * Duration::Minute +
	         7001 * Duration::Millisecond},
...
TEST_F(TimeUTest, HasMonotonicClock) {
	...
	std::vector<TestData>       data = {
	          ...
	          {Time::Now().Add(2 * Duration::Nanosecond),
	           true,
	           Time::SYSTEM_MONOTONIC_CLOCK},
    };

	for (const auto &d : data) {
		EXPECT_EQ(d.T.HasMono(), d.OK);
		if (d.OK == true) {
			EXPECT_NO_THROW({ EXPECT_EQ(d.T.MonoID(), d.Expected); });
		} else {
			EXPECT_THROW({ d.T.MonoID(); }, std::exception);
		}
	}
}
...
TEST_F(TimeUTest, TimeSubstraction) {
	...
	google::protobuf::Timestamp zero =
	    google::protobuf::util::TimeUtil::TimeTToTimestamp(0);
	google::protobuf::Timestamp oneSecond =
	    google::protobuf::util::TimeUtil::TimeTToTimestamp(1);
	google::protobuf::Timestamp twoSecond =
	    google::protobuf::util::TimeUtil::TimeTToTimestamp(2);
	...
	std::vector<TestData> data = {
	    ...
	    },
	    {
	        Time::FromTimestampAndMonotonic(
	            twoSecond,
	            (2 * Duration::Second + 1).Nanoseconds(),
	            1
	        ),
	        Time::FromTimestamp(oneSecond),
	        1 * Duration::Second,
	    ...
	    },
	    {
	        ...
	        Time::FromTimestampAndMonotonic(
	            oneSecond,
	            (1 * Duration::Second - 1).Nanoseconds(),
	            1
	        ),
	        1 * Duration::Second,
	        '>',
	...
	for (const auto &d : data) {
		...
		case '>':
			...
			break;
		default:
			ADD_FAILURE() << "Intern test failure: unknown comparison test '"
			              << d.Comp << "'";
		}
	}
}
...
TEST_F(TimeUTest, Overflow) {
	...
	EXPECT_THROW(
	    {
		    google::protobuf::Timestamp a;
		    auto                        t = Time::FromTimestampAndMonotonic(
                a,
	...
	b.set_seconds(0);
	EXPECT_NO_THROW({ Time::FromTimestamp(a).Sub(Time::FromTimestamp(b)); });
	...
	EXPECT_THROW(
	    {
		    Time::MonoFromSecNSec(
		        std::numeric_limits<uint64_t>::max() / 1e9,
		        1e9
		    );
	    },
	    Time::Overflow
...
TEST_F(TimeUTest, InfiniteTimeComputationOverflow) {
	...
	EXPECT_NO_THROW({ Time::Forever().Add(0); });

	EXPECT_THROW({ Time::Forever().Add(1); }, Time::Overflow);

	EXPECT_THROW({ Time::Forever().Add(-1); }, Time::Overflow);

	EXPECT_NO_THROW({ Time::SinceEver().Add(0); });

	EXPECT_THROW({ Time::SinceEver().Add(1); }, Time::Overflow);

	EXPECT_THROW({ Time::SinceEver().Add(-1); }, Time::Overflow);

	EXPECT_THROW({ Time::Forever().Sub(Time()); }, Time::Overflow);

	EXPECT_THROW({ Time::SinceEver().Sub(Time()); }, Time::Overflow);
}
...
TEST_F(TimeUTest, TimeIO) {
	...
	std::vector<TestData> data = {
	    {((2 * 365 * 24 + 5) * Duration::Hour + 20 * Duration::Second)
	             .Nanoseconds() /
	         1000000000LL,
